...
TEST(DataHeaderTest, SingleThreadWriteConflict) {
  Transaction txn1(TransactionType::WRITE, 1);
  Transaction txn2(TransactionType::WRITE, 1);
  DataHeader header;
  ...
  std::string val;
  bool not_found;
  ASSERT_TRUE(header.Select(&txn1, &val, &not_found));
  ASSERT_FALSE(not_found);
  ASSERT_EQ(val, "1");
  ASSERT_FALSE(header.Select(&txn2, &val, &not_found));
  ASSERT_FALSE(not_found);
  ASSERT_EQ(val, "1");
